...
const DatePicker: React.FC<Props> = ({ onChange, label, subtext }) => {
            ...
            <DatePickerContainer ref={calendarRef} onClick={()=>{setIsCalendarOpen(true)}} className={selectedDate ? 'success' : ''}>
                ...
                {isCalendarOpen && (
                    <div style={{position:"absolute",marginTop:4}}>
                        <Calendar onChange={handleDateChange} selected={selectedDate} hideCalendar={()=>{}}/>
                    </div>
                )}
            </DatePickerContainer>
        </div>
...
interface CalendarProps {
    onChange: (date: Date) => void
    selected: Date | null
    hideCalendar: (a:boolean) => void
}

const Calendar: React.FC<CalendarProps> = ({ onChange, selected ,hideCalendar}) => {
    ...
    const renderCalendar = (): JSX.Element[] => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const isCurrentDay = day === currentDay && month === currentDate.getMonth() && year === currentDate.getFullYear();
